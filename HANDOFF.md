...
- 라이선스 검사 수정·회귀 테스트·CI 연결은 `fe3809e`에 커밋했다.
- `python3 ops/tests/runtime-license-policy-test.py`의 테스트 6개가 통과했다.
  정상 의존성과 기존 예외는 허용하고, 미승인 LGPL·GPL 예외·AGPL 전용 의존성,
  허용 라이선스를 잃은 승인 패키지, 다른 패키지의 예외 재사용, 승인 버전 변경과
  미승인 라이선스 정보 누락은 차단한다. 실패 시 제외 목록을 출력하지 않는 것도 확인했다.
- 실제 부트 JAR에서 고정 Trivy 0.74.0으로 생성한 SBOM 구성요소 76개가 수정한
  정책과 Trivy 라이선스 검사를 통과했다. 대조 검사에서는 LGPL 제외를 빼면
  Logback 두 패키지가 검출되고 실패하는 것을 확인했다. 이번 검사는 라이선스에
  한정했으며 취약점 스캔과 배포 이미지 빌드는 재실행하지 않았다.
- `actionlint` 1.7.12와 `git diff --check`를 통과했다. 새로운 Python·Gradle
  의존성은 추가하지 않았고 라이선스 허용 목록·승인 예외·의존성 체크섬도 변경하지 않았다.
- Java 코드와 Gradle·Compose 설정을 변경하지 않아 전체 Java 테스트는 반복하지
  않았다. 직전 경보 집계 `ccf7315`·백업 중단 처리 `bf61e1b` 검증에서 일반 테스트
  370개와 기본 100개 모니터 부하·복구 2개, 부트 JAR의 Apache-2.0 전문 검증을
  실패·오류·건너뜀 없이 통과한 결과가 최신 전체 검증이다.
- 직전 검증에는 백업·복원 통합 테스트 5개, 경보 규칙 8개, 백업 셸의 Bash 구문과
  ShellCheck가 포함됐다. 부하 시험의 최종 백로그는 0이었지만 실제 네트워크·운영
  폴링·Hikari 풀·프로세스 재시작 부하의 증거는 아니다.
