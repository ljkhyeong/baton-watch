...
- 2026-08-30 추가 검토에서 확인한 점검·전달 카운터 동시 입력의 경보 평가 오류와
  백업 도구 중단 시 하위 프로세스·임시 환경이 남는 문제를 수정했다.
...
- DB 백업·복원 통합 테스트 5개에서 모니터·결과·재시도 중인 이벤트 복원,
  `0600` 권한과 덮어쓰기 거부, 손상된 아카이브·백로그 불일치 거부, 실패한 덤프의
  미완성 파일 정리를 확인했다. 추가한 2개 사례는 응답하지 않는 Docker CLI를
  대역으로 실행한 뒤 부모 셸에만 `SIGTERM`을 보내 하위 프로세스 종료, 임시 파일
  제거와 복원 환경 정리 명령 호출을 확인한다. 실제 복원 시험의 임시 컨테이너가
  남지 않은 것도 확인했다.
- `./ops/tests/prometheus-rules-test.sh`로 경보 8개의 문법과 지속 장애·복구,
  같은 레이블의 점검·전달 카운터 동시 입력과 합계 임계치, 비활성 전달·카운터
  초기화·수집 대상 누락을 검사했다. 예약 실패 테스트는 실제
  Prometheus 메트릭 이름과 레이블도 확인한다.
- 수정한 백업 셸의 Bash 구문과 ShellCheck 검사를 통과했다. CI의 actionlint와
  스테이징 Compose 정책·렌더링은 이전 도구 구현에서 검증했고 이번에는 변경하지 않았다.
