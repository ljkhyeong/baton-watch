...
- 작업 브랜치: `codex/simplify-watch-skills`. 본문을 읽지 않는 점검에 남아 있던 바이트 계산·초과 검사와
  전용 테스트 2개, 사용하지 않는 `ClaimedCheck.scheduledAt`과 스케줄러 메트릭 주입을 제거했다.
  기존 설정 검증·결과 저장 필드, 헤더·콜백 본문 제한, DB 예약·리스·지연 측정은 유지한다.
...
| 대상 | 결과와 재사용 범위 |
| --- | --- |
| 점검 코드 정리 | 관련 테스트 후 `./gradlew test` 통과. 435개 중 423개 실행·domain 12개 기존 결과 재사용, 실패·건너뜀 없음. 실제 PostgreSQL 검사 포함 |
