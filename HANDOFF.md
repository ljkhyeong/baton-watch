...
- 별도 포트의 Actuator 상태·Prometheus 엔드포인트와 낮은 카디널리티의 `baton.watch.check.attempts{outcome}`·`baton.watch.check.duration{outcome}` 점검 메트릭, 최근 선점 배치의 최대 일정 지연 게이지, 스케줄러·백로그·가장 오래된 미전달 이벤트의 경과 시간·완료 처리·전달 결과 텔레메트리가 구성되어 있으며, Compose는 관리 포트를 공개하지 않는다. 일정 지연은 선점 쿼리가 이미 읽은 `next_check_at`과 데이터베이스 선점 시각의 차이를 사용하고 선점이 없는 배치에서는 0으로 갱신한다. 외부 알림 스택, 프런트엔드, 브로커, 운영 배포는 존재하지 않는다. 스케줄러 오류는 Spring `tasks.scheduled.execution` 타이머를 사용하며, 실패는 비식별화 핸들러가 클래스만 기록하고 고정 지연 작업을 계속 예약하기 전에 해당 관측에 도달한다. 저장소 내 소비자가 없는 중복 WATCH 소유 스케줄러 실패 카운터는 제거되었다. 관리 허용 목록은 정확히 `health,prometheus`로 유지되며, Spring 내부 작업 진단이 원본 예외를 보유하므로 `scheduledtasks`는 노출하지 않는다.
- GitHub Actions `Verify` 작업은 전체 SHA로 고정된 공식 checkout·Java·Gradle·Dependency Review·CodeQL 액션을 사용하고, 공식 SHA-256으로 고정된 Gradle Wrapper와 해석된 빌드·테스트 의존성의 SHA-256 검증 메타데이터를 적용하며 Docker를 요구한다. 호스트 Gradle 실행은 캐시 없는 깨끗한 전체 테스트를, 최종 WATCH Docker 이미지 빌드는 실행 가능한 부트 JAR 생성을 소유한다. 필수 PostgreSQL·운영 루트 컨텍스트 실행 증거, ShellCheck, 스테이징 Compose 정책, 빠른 역할 초기화·마이그레이션 생성 검사, 전달 사전검사 호출 계약과 실제 PostgreSQL 역할·마이그레이션·비루트 최종 WATCH 런타임 스모크를 실패 폐쇄 방식으로 요구한다. Pull Request에서는 새 의존성의 취약점과 라이선스를 검토하고 모든 `Verify` 실행에서 CodeQL Java 분석을 수행한다. Jackson 3.1.5, Apache HttpClient 5.6.3, pgJDBC 42.7.12를 명시적으로 고정했고 Gradle·GitHub Actions·Docker 주간 Dependabot 설정을 추가했다.
...
- 저장소 Actions 정책은 GitHub 소유 액션과 전체 SHA로 고정된 `gradle/actions/setup-gradle` 액션만 허용하고 전체 커밋 SHA 참조를 요구한다. 워크플로 토큰은 저장소 콘텐츠를 읽기 전용으로 유지하고 CodeQL 결과 업로드에 필요한 `security-events: write`만 추가한다. 외부 기여자의 포크 워크플로 실행 전에 승인을 요구하며, 비밀값 검사와 푸시 보호가 활성화되어 있고 열린 비밀값 경고는 없다.
- 저장소는 표준 영문 전문을 보존한 Apache License 2.0으로 배포한다.
...
- Gradle 9.2.1에서 의존성 검증 메타데이터를 생성한 뒤 `./gradlew clean test :bootstrap:bootJar --no-daemon --no-build-cache`를 재실행했다. 전체 341개가 실패·오류·건너뜀 없이 통과했고, 모듈별로 `domain` 8개, `application` 22개, `adapter-in-web` 19개, `adapter-out-external` 203개, `adapter-out-persistence` 36개, `bootstrap` 53개다. 로컬 캐시와 별개인 깨끗한 Docker 빌드도 검증 메타데이터를 적용해 실행 가능한 부트 JAR를 만들었다.
