...
- 좁은 범위의 전달 조립 테스트 14개와 점검·전달 예약 테스트 14개가 통과했다.
  활성·비활성 표현과 공백을 포함한 설정 바인딩, 전체 전달 빈 등록, 잘못된 값의
  시작 실패와 API 토큰 재사용 거부를 확인했다. 점검을 끈 상태에서 전달을 켜면
  예약 6개, 함께 끄면 유지보수 예약 5개가 등록된다.
- 진단 테스트는 조회 전후 DB 불변과 출력 계약을 유지하며, 기본 로그 설정과
  상세 SQL·매개변수 로깅을 켠 설정 모두에서 정상·잠금 실패 조회의 식별자 비노출을
  PostgreSQL 서버 로그까지 확인한다. 오류·실행 시간 로그 자체는 유지된다.
- `./gradlew test :bootstrap:verifyBootJarLicense --no-daemon --no-build-cache
  ...
  컴파일·실행했으며 일반 JUnit 60개 스위트·420개 테스트에서 실패·오류·건너뜀은 없다.
  부트 JAR의 Apache-2.0 라이선스도 확인했다. 의존성·스키마 변경은 없다.
- 전체 검증 후 사용하지 않는 설정 도우미를 제거하고 `:bootstrap:test`를 다시 실행했다.
  해당 모듈의 19개 스위트·87개 테스트도 실패·오류·건너뜀 없이 통과했다.
