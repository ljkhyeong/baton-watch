...
- 좁은 범위의 점검 예약 테스트 12개와 실제 PostgreSQL 진단 테스트 9개가 통과했다.
  설정 객체의 값과 예약 작업 등록의 일치, 잘못된 불리언 값의 시작 실패,
  점검 비활성 시 전달·유지보수 예약 6개 유지를 확인했다.
...
- `./gradlew test :bootstrap:verifyBootJarLicense --no-daemon --no-build-cache
  ...
  컴파일·실행했으며 일반 JUnit 60개 스위트·407개 테스트에서 실패·오류·건너뜀은 없다.
  부트 JAR의 Apache-2.0 라이선스도 확인했다. 의존성·스키마 변경은 없다.
- 별도 네트워크 없는 임시 PostgreSQL에서 로그 보호 설정 권한을 제거했을 때
  실제 CLI가 조회 전에 실패하고 서버 로그에도 리소스 참조가 남지 않음을 확인했다.
  임시 컨테이너는 정리했으며 운영 DB는 조회하거나 변경하지 않았다.
