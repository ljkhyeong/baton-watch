...
- 작업 브랜치는 `codex/remove-residual-duplication-20260820`이며, 공개 PR은
  [#23](https://github.com/ljkhyeong/baton-watch/pull/23)이다.
- Java 21 / Spring Boot 4.1.0 기반 모니터링 MVP와 하나의 BATON HTTPS 상태 변경
  콜백 전달이 구현되어 있다.
...
- `./gradlew clean test :bootstrap:verifyBootJarLicense --no-daemon --no-build-cache`가
  358개 테스트와 실행 가능한 부트 JAR의 Apache-2.0 전문 검증을 통과했다.
...
1. PR #23의 최신 `Verify / verify` 결과를 확인한다.
