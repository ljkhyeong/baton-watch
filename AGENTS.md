...
- 작업 시작 커밋을 기록한다. 파일을 작성·수정한 직후 `python3 ops/check-feedback.py file <파일...>`로 해당 파일의 형식·문법·Java 컴파일을 검사한다. 한 번에 수정한 관련 파일은 묶어서 검사하고, 실패하면 해당 범위를 고친 뒤 다음 작업으로 넘어간다.
- 완료 직전 `python3 ops/check-feedback.py finish --base <작업 시작 커밋>`을 검증 기록 도구로 실행한다. 커밋 여부와 관계없이 전체 변경·새 파일을 검사하고, Java·빌드 변경은 ArchUnit으로 전체 계층 의존성을 확인한다. 같은 기준의 전체 `git diff`와 새 파일 본문을 읽어 구조·중복·범위도 검토한다.
