...
| 장애 진단·복구 | 점검·전달 진행 상태와 최근 이력의 읽기 전용 조회, DB 백업·격리 복원, BATON 스냅샷 대조·재전송 |
...
점검기의 자원 설정에도 상한을 적용합니다. 잘못된 유니코드 URL은 등록·리다이렉트 전에 거부하며,
경로·쿼리의 정상 한글·이모지는 유지합니다. 대상 점검은 리다이렉트마다 주소를 다시 검증하고,
...
검사 명령이 성공해도 SBOM 보고서가 없거나 비어 있으면 실패로 처리하며, 완료 체크섬을 생성하지 않습니다.
