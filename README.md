...
엔드포인트는 포트 `443`의 절대 공개 글로벌 HTTPS URL이어야 하며 사용자 정보, 쿼리, 프래그먼트, IP 리터럴 호스트를 포함할 수 없습니다. BATON은 Bearer 토큰을 인증하고 `Idempotency-Key`와 `eventId`가 일치하는지 확인해야 합니다. `2xx`로 응답하기 전에 처리한 `eventId`를 DB에 기록해 중복 처리를 막아야 합니다. 정확한 페이로드와 재시도 동작은 전달 계약을 참고합니다.
