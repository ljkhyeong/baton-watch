...
- 낮은 카디널리티의 점검 시도·소요 시간 `outcome` 메트릭, 현재 실행 중인 점검 수, 최근 점검 배치의 최대 일정 지연, Spring 예약 실행 타이머, 이벤트 전달 백로그·가장 오래된 미전달 이벤트의 경과 시간·완료 처리·제한된 결과 메트릭
