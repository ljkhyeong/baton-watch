...
BATON WATCH는 BATON `RoleResource`에 등록된 URL의 연결 상태를 점검하는 독립 서비스다. 모니터링 지연과 실패가 BATON 트랜잭션 및 프로젝션에 영향을 주지 않도록 그 바깥에서 처리해야 한다.

## 현재 기능

PRD-0003의 모니터링 MVP는 구현되어 있다. WATCH는 리비전이 있는 활성 또는
비활성 URL 스냅샷을 받아 제한된 비동기 검사를 예약하고 실행하며,
점검 시도·결과를 DB에 저장하고 현재 상태를 계산한다. 상태가 바뀌면 이벤트도 DB에 기록한다. 구성된 단일 BATON HTTPS 콜백으로 전달하는 PRD-0004도
...
대기·실행 중 점검에 합류하고 리소스별 30초 간격을 적용한다. 전체 점검의 실시간
