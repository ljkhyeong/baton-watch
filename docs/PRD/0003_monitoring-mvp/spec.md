...
- `baton.watch.check.schedule.delay` 게이지는 최근 선점 배치에서
  `next_check_at`부터 데이터베이스 선점 시각까지의 최대 지연을 초 단위로
  기록한다. 선점 작업이 없는 배치에서는 0으로 갱신하며 식별자 레이블을
  사용하지 않는다.
...
10. 현재 실행 중인 점검 수, 최근 선점 배치의 최대 일정 지연, 점유·완료 처리·
    만료 리스 회수와 JVM-PostgreSQL 시계 편차를 식별자 없이 관측할 수 있다.
