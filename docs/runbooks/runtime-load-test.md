...
4. 점검 완료를 잠시 보류하고 시험 DB의 다음 점검 시각만 2분 전으로 조정한다.
   별도 유지보수 작업이 `baton_watch_check_schedule_delay_seconds`에 지연을
   기록하는지 확인한다.
5. 점검을 재개해 예약 작업이 모든 결과와 상태 변경 이벤트를 저장하는지
   확인한다.
...
허용하는 최솟값 이상에서 시험용으로 줄이며, 운영 기본값이나 스키마는
변경하지 않는다.
