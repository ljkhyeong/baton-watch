...
전용 공개 스테이징에서 WATCH가 BATON에 이벤트를 전달하는지 확인합니다.
최초 전달, 응답 유실 후 같은 이벤트 ID로 재전달, 쌓인 미전달 이벤트의 처리를 검증합니다. 운영 배포 승인은 별도로 받아야 합니다.
...
- BATON 수신기 앞에서 장애를 재현할 스테이징 전용 프록시. 요청 하나를
...
사전 점검은 토큰 형식과 두 토큰의 분리를 확인한 뒤 공개 WATCH 상태를 조회합니다.
HTTP `200`과 JSON의 `service: baton-watch`, `status: UP`을 모두 요구합니다.
다른 서비스·HTML·빈 본문·잘못된 JSON·비정상 상태이면 BATON 수신기 요청 전에 중단합니다.
JSON 필드가 중복되면 값이 같거나 이름을 이스케이프로 적어도 실패합니다.
공개 스모크 검사와 같은 [상태 판정 코드](../../ops/check-watch-status.py)를 사용합니다.

이어서 인증 없이 잘못된 JSON을 콜백으로 POST합니다. 수신기는 반드시 `401`을 반환해야 합니다.
JSON 파싱 오류에 해당하는 4xx이면 인증이 역직렬화보다 먼저 수행됐음을 확인할 수 없어 실패합니다.
입력이 잘못되면 HTTP 요청을 보내지 않으며, 정상 경로에서도 요청은 두 번뿐입니다.

두 토큰은 하위 Python·curl 환경에서 제거하고 전송하지 않습니다. 각 응답은 64 KiB로 제한합니다.
WATCH 상태 본문은 소유자만 접근할 수 있는 임시 파일에서 읽고 성공·실패 종료 시 삭제합니다.
콜백 응답 본문은 저장하지 않으며, 두 응답 모두 출력하지 않습니다. 수신 실패도 사전 검사 실패입니다.
공통 URL 정책은 IP 리터럴과 정수·8진수·16진수·축약 IPv4 표기를 요청 전에 거부합니다.
기존 HTTPS·TLS 검증·프록시 미사용·리다이렉트 비추적·연결 5초·전체 10초 제한은 유지합니다.

회귀 검사는 상태 JSON 판정, 후속 요청 중단, curl 요청 조건, 임시 파일 삭제와 오류 출력을 확인합니다.
유효한 토큰의 인증 성공, 배포된 WATCH 설정, 실제 이벤트 전달은 별도로 검증해야 합니다.

## 이벤트 전달용 Compose 설정 추가
...
참조, 페이로드, 토큰, 헤더, 예외 메시지 또는 원본 로그는 검증 자료로 보관하지
마세요.
...
백로그 게이지는 1분 주기의 유지보수 일정에 따라 갱신되므로, 게이지에 최신 상태가 반영되기를
기다리는 동안에는 PostgreSQL 행 상태를 기준으로 판단합니다. 카운터 시계열이
없다면 최초 증가 전 값은 0을 의미합니다.

스케줄러의 `replayed` 로그 필드로는 BATON 수신함의 재전달 여부를 확인할 수 없습니다. 이미
전달된 이벤트를 발견한 WATCH 완료 처리 횟수를 셉니다.

장애 시험용 프록시는 최초 BATON `202` 수신 결과와 재전달 수신 결과를 메모리에서
비교하고 다음과 같이 정해진 항목의 집계 결과만 노출해야 합니다. 요청 수, 고유
...
1. 스테이징 장애 시험용 프록시를 그대로 전달하는 모드로 설정하고 제한된
   카운터를 초기화합니다.
2. `baton-manager:<namespace>:role-resource:<uuid>` 형식의 고유한 정규 참조를
   만들고, 통제된 공개 대상을 향하는 소스 리비전 1의 ACTIVE 모니터를
   동기화합니다. 이 전달 시험에서는 WATCH 동기화 API를 직접 호출해도 됩니다.
   전체 연동을 검증할 때는 정상적인 BATON 커밋 후 동기화 경로를 사용하세요.
...
운영자 전용 PostgreSQL 세션에서 실행할 수 있는 WATCH 검증 결과 조회 쿼리는 다음과
같습니다.
...
두 차례 업스트림 관찰과 `sameReceipt` 비교로 재전달 여부를 확인합니다.

## 3단계: 백로그 해소

1. 장애 시험용 프록시가 전달하지 않고 `503`을 반환하는 모드로 설정합니다.
...
정해진 항목의 집계 결과만 보관할 수 있습니다.

## 정리 및 롤백

장애 시험용 프록시는 항상 그대로 전달하는 모드로 복원하세요. 모든 임시 모니터를
