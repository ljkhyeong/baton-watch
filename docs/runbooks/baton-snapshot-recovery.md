...
응답 JSON의 필드가 중복되어도 해당 요청을 실패로 처리한다. 값이 같거나 이름을 이스케이프로 적은
중복도 거부하며, 묶음 안의 모니터 객체에도 적용한다. 모호한 GET 뒤에는 해당 항목을 PUT하지 않고,
모호한 PUT 응답은 `REPLAYED`로 보고하지 않는다. 결과는 `LOOKUP_OR_REPLAY_FAILED`이며
`remoteRevision`은 `null`이다. 나머지 항목은 계속 처리한다.
