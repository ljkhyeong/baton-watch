...
상태: 운영자용 배포 절차서. 실제 배포·가동 여부는 별도 확인 필요
...
없습니다. 내부·외부 스모크 테스트 결과를 모두 보관하세요.
...
부트 JAR 라이선스 검증을 수행하고, 최종 WATCH Docker 이미지 빌드는 이미지에 포함할
실행 가능한 부트 JAR을 생성합니다. 배포 호스트에서는 다시 빌드한 현재 플랫폼의
이미지 ID·OCI 리비전·아카이브 체크섬을 먼저 검증 기록으로 보관하고, 그 아카이브를
공용 스크립트가 직접 검사합니다. 검사 명령이 성공해도 해당 SBOM 보고서가 없거나 빈 파일이면 실패합니다.
한 산출물의 취약점 검사가 실패해도 나머지 이미지와
...
반환합니다.
로그가 비어 있거나 공백뿐이면 감사 실패입니다. 비밀 파일도 각각 검사하므로 하나라도
값이 없으면 실패하며, 금지 값 파일에도 검사할 값이 필요합니다. 입력 누락은 수집 대상·기간과 파일 내용을 확인한 뒤 다시 검사하세요.
