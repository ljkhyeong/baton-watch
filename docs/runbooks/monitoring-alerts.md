...
12개 패널은 수집 상태, 최대 점검 지연 시간, 미전달 이벤트 수·최대 대기 시간, 결과별 초당 시도 수,
평균 HTTP 소요 시간, 완료 처리 실패 건수, 리스 회수, 예약 완료 횟수, 애플리케이션·DB 시간 차이,
DB 연결 풀, 진행 중인 외부 요청 수를 보여준다. 임계치는 운영 승인 없이 자동 알림으로 설정하지 않는다.
...
| `WatchMaintenanceStalled` | 위 조건에서 유지보수 여섯 메서드 중 하나라도 실행 완료 없음 | 2분 | 유지보수 스케줄러, DB 대기, 백로그 지표 갱신 |
...
| `WatchCheckScheduleDelayed` | 점검 활성 인스턴스에서 가장 오래된 선점 가능 점검의 일정 지연이 120초 초과 | 5분 | 대상 지연과 처리할 모니터 수 |
...
일정 지연은 유지보수 주기마다 DB에서 읽은 **가장 오래된 선점 가능 점검**의
값이다. 활성 리스와 아직 도래하지 않은 일정은 제외하며 선점 가능한 점검이 없으면
0이다. 작업 중단은 별도의 미실행 경보로 감시한다. 수집 대상 하나만 사라지고 다른 인스턴스가
...
감시 스레드·자체 heartbeat 지표는 추가하지 않는다. 할 일이 없는 빈 배치도
...
`purgeDeliveredEventHistory`, `refreshEventDeliveryBacklog` 여섯 메서드 모두의 증가량이 있어야 정상이다.
메서드별로 합산한 뒤 개수를 세므로 한 작업의 성공·오류 시계열이 다른 작업의
중단을 가리지 않는다.

앞의 네 유지보수 메서드는 `MonitoringMaintenanceScheduler`, 뒤의 두 메서드는
`EventDeliveryMaintenanceScheduler`의 `code_namespace`로 구분한다. 점검 중지 중에도
유지보수 여섯 메서드와 활성화된 이벤트 전달의 미실행 경보는 계속 평가한다.
...
기존 수집 경보가 담당한다. 기본 설정에서 마지막 실행 횟수 증가 후 약 5분간 새 실행이 없고, 이 상태가
2분간 유지되면 경보가 발생하며 실제 지연에는 수집·평가 간격이 더해진다.
...
- 미실행 규칙은 기본 점검·전달 폴링 1초와 유지보수 1분을 기준으로 한다.
  예약 주기를 늘리면 `[5m]` 조회 구간을 실제 주기·허용 실행 시간·수집 간격보다
...
- `process_uptime_seconds`와 예약 실행 메트릭의 `code_namespace`,
  ...
  메서드를 추가·삭제·변경하면 규칙의 메서드 목록과 기대 개수 `6`을 같이 갱신한다.
...
- 실제 장애·복구 시험으로 임계치, 경보 지연과 수신·해제를 확인한 뒤 배포 검증 결과를
