...
끊기거나 공개 터널 경로가 잘못되어도 이 상태 확인은 성공할 수 있다. WATCH의
...
8초다. HTTP 200과 JSON `service: baton-watch`, `status: UP`을 함께 요구하며
읽는 본문은 16 KiB로 제한한다.
리다이렉트를 따라가지 않으며 공개 모듈은 TLS를 필수로 사용하고 기본 인증서 검증을
유지한다. IPv4만 검사하므로 IPv6 가용성을 증명하지는 않는다.

공개 상태 API는 본문 없는 `HEAD`도 지원한다. 상태 코드·헤더만 확인하는 도구에서 사용할 수 있다.
이 템플릿은 서비스 이름과 `UP`을 본문에서 확인하므로 기존 `GET`을 유지한다.
`HEAD`도 상태 경로의 같은 요청 수 제한을 적용받는다.
...
아닌 지속 실패에만 경보가 발생한다. 이 점검 요청도 NGINX 상태 경로의 요청 제한에 포함된다.
...
실제 WATCH·공개 TLS·Cloudflare·알림 도착을 시험한 것은 아니다.
