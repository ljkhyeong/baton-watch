...
검증 도구에 `SIGINT`·`SIGTERM`·`SIGHUP`가 오면 실행한 명령의 프로세스 그룹에 같은 신호를 전달한다.
직접 실행한 명령의 종료를 최대 5초 기다린 뒤 같은 그룹에 남은 하위 명령을 강제 종료한다.
중단 결과는 `failed`와 종료 코드 130·143·129로 기록하며, 하위 명령이 정리 후 0으로 종료해도 성공으로 처리하지 않는다.
별도 세션의 데몬·컨테이너 정리는 실행한 명령이 담당한다. 검증 도구 자체의 `SIGKILL`·서버 중단은 결과 기록을 보장하지 못한다.
