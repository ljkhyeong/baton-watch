...
점검 중지·재개, 재점검 요청과 장애 진단 절차다. 운영자가 직접 실행하고
실제 점검·BATON 콜백 전달 결과를 확인해야 한다.
...
3. 현재 배포와 같은 이미지·볼륨·추가 Compose 설정을 사용해 WATCH만 재생성한다.
...
스테이징에서는 [배포 절차](staging-deployment.md)의 현재 이미지·볼륨·추가 Compose 설정을
...
HTTP 202 응답은 다음 세 가지로 구분한다.

| 상태 | 의미 |
| --- | --- |
| `SCHEDULED` | 다음 점검 일정을 앞당김 |
| `ALREADY_SCHEDULED` | 이미 대기 중인 점검을 사용 |
| `IN_PROGRESS` | 현재 실행 중인 점검을 사용 |

요청만으로 상태나 실패 횟수가 초기화되지 않는다. 진행 상태는 모니터 GET의 `checkStatus`,
결과는 `lastCheckedAt`과 `lastOutcome`으로 확인한다. 위 POST의 `status`는 요청 접수 결과다.
요청 직전에 시작된 점검을 사용할 수도 있어, 새 네트워크
요청이 반드시 발생하는 것은 아니다.

실행할 활성 작업자가 없으면 요청은 대기한다. 비활성 점검 대상은 409로 거부한다.
새 수동 예약은 직전 예약 후 30초부터 가능하며 429이면 `Retry-After`만큼 기다린다.
본문 필드, 오류와 응답 의미는 [API 계약](../PRD/0002_api-contract/spec.md)을 따른다.

## 리소스별 최근 장애 확인

모니터 API가 `503 SERVICE_UNAVAILABLE`을 반환하면 `Retry-After`의 5초 이상 기다린다.
계속 실패하면 DB 연결·연결 풀·잠금 대기를 확인한다. 503을 링크 장애나 미등록 자료로 처리하지 않는다.
PUT은 같은 리비전·본문으로 다시 보내고, 재점검 POST의 429에는 해당 응답의 대기 시간을 따른다.
5초 안에 복구되거나 요청이 DB에 반영되지 않았다는 뜻은 아니다.
...
| --- | --- |
| `INACTIVE` | 비활성 대상 |
| `IN_PROGRESS` | 작업자의 점유가 아직 만료되지 않은 점검 |
| `SCHEDULED` | 유효한 점유가 없고 다음 점검 시각 전인 대상 |
| `QUEUED` | 유효한 점유가 없고 점검 시각이 되어 실행을 기다리는 대상 |
...
- 시각은 UTC, `durationSeconds`는 소수 초다. 원본 URL·호스트·본문·리소스 참조·리스 토큰·만료 시각은 출력하지 않는다.
  시도·이벤트 ID는 관련 기록을 찾는 데 사용하며 메트릭 레이블에 넣지 않는다.
