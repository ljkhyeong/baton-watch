...
발생한다. BATON이 이 점검을 동기로 수행하면 자료 변경·조회가 불안정한 외부 네트워크 응답을 기다리게 된다.
...
`UNKNOWN`/`HEALTHY`/`DEGRADED`/`BROKEN` 상태와 DB에 저장한 상태 변경 이벤트를
관리한다. BATON은 RoleResource의 원본 데이터와 인가 권한을 유지한다. BATON은
...
선점 트랜잭션, SSRF·DNS 리바인딩 방어와 리다이렉트·시간·바이트 제한을 적용한
외부 점검, 짧은 확정 트랜잭션을 차례로 사용한다. 확정 작업은 응답 본문이 아닌
메타데이터만 저장하며, 상태가 변경되면 같은 트랜잭션에서 이벤트를 DB에 기록한다.
시간은 `Clock`을 통해 공급한다. 메트릭에는 값의 종류가 제한된 결과 레이블만
사용하고 원본 URL은 절대 사용하지 않는다.

현재 구성에는 프런트엔드나 메시지 브로커가 없다. PRD-0004와 ADR-0003은
...
동기화, 리스 복구, 멱등적인 점검 확정, BATON의 이벤트 ID 중복 제거,
