...
스테이징 터널용 Compose 설정에 공식 NGINX OSS 이미지를 버전·다이제스트로 고정해 추가한다.
...
자체 요청 제한 응답에는 `Retry-After: 1`을 붙여 최소 재시도 간격을 안내한다.
`@rate_limited`에만 `add_header ... always`를 적용하므로 WATCH가 반환한 오류의
대기 시간은 그대로 전달한다. 고정 안내 값이며 혼잡 해소나 다음 요청의 성공을 보장하지 않는다.
